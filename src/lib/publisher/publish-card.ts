...
export async function publishCard(orderId: string): Promise<string> {
  ...
  try {
    ...
    if (order.status === "published") {
      ...
      return `${process.env.PUBLIC_CARD_BASE_URL}/cards/${order.cardId}/index.html`;
    }
    ...
    // Final published card URL: https://pub.dearnote.asia/cards/{cardId}/index.html
    const cardUrl = `${cleanBaseUrl}/cards/${cardId}/index.html`;
    ...
    if (draft.voiceNote) {
      ...
      console.log(`[Publisher] Copying voice note: ${sourceKey} -> ${destKey}`);
      await copyObject(sourceKey, destKey);
    }

    // 6. Generate static index.html content
    console.log(`[Publisher] Rendering index.html for card ${cardId}...`);
    const htmlContent = generateIndexHtml(publishedConfig);

    // 7. Write index.html, config.json, qr.svg, qr.png to cards/{cardId}/ in R2
    console.log(`[Publisher] Writing static assets to R2 bucket...`);
    const cardFolderPrefix = `cards/${cardId}`;

    await putObject(`${cardFolderPrefix}/index.html`, htmlContent, "text/html");
